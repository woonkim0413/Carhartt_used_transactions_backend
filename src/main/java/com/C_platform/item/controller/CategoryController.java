...
public class CategoryController {
    ...
    public ResponseEntity<ApiResponse<List<CategoryResponseDto>>> getCategories() {
        Optional<List<CategoryResponseDto>> categories = categoryService.getCategories();
        MetaData meta = MetaData.builder()
                .timestamp(LocalDateTime.now())
                .build();
        return categories.map(categoryResponseDtos -> ResponseEntity.ok(ApiResponse.success(categoryResponseDtos, meta)))
