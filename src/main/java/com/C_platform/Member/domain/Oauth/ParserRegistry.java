...
public class ParserRegistry {
    private final Map<Provider, UserInfoParser> byProvider;

    // (중요)
    // ParserRegistry가 bean으로 등록되어 있기에 spring이 bean으로 등록된 모든
    // UserInfoParser들을 가져와서 List로 묶은 뒤 arg로 넣어준다
    public ParserRegistry(List<UserInfoParser> parsers) {
        ...
        this.byProvider = parsers.stream()
                .collect(Collectors.toUnmodifiableMap(UserInfoParser::getProvider, p -> p));
    }

    public UserInfoParser of(Provider provider) {
